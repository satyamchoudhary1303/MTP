...
% Physical parameters
rv = 0.6; % [m]  Aerodynamic arm effective length
...
% Initial conditions: [theta0 (rad); omega0 (rad/s)]
theta0 = deg2rad(45); % 10 degree initial deflection
omega0 = 0; 
y0 = [theta0; omega0];
...
data = readtable('1000rpm_45deg.csv');  % Replace with your file name
% Filter experimental data to 2 seconds
valid_indices = data.Time <= 2;
t_exp = data.Time(valid_indices);
theta_exp = data.Angle(valid_indices);


% Plot both
figure;
plot(t, theta_deg, 'b', 'LineWidth', 1); hold on;
plot(t_exp, theta_exp, 'r', 'LineWidth',1);
xlabel('Time (s)');
ylabel('Angle (degrees)');
title('Comparison: Simulated vs Experimental Wind Vane Angle at 1000 rpm');
